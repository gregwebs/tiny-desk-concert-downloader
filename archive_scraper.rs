...
fn scrape_archive(year: &str, month: &str, day: Option<&str>) -> Result<()> {
    ...
    let url = format!("https://www.npr.org/series/tiny-desk-concerts/archive?date={}-{}-{}", 
    ...
    for article in document.select(&article_selector) {
        ...
        if let (Some(title_el), Some(time_el), Some(teaser_el)) = (title_element, time_element, teaser_element) {
            ...
            let full_teaser_text = teaser_el.inner_html().trim().to_string();
            let date_text = time_el.inner_html().trim().to_string();
            ...
            if !date_text.is_empty() {
                ...
                clean_teaser = full_teaser_text.replace(&date_text, "")
    ...
    println!("Found {} Tiny Desk Concerts for {}", concerts.len(), display_date);
    
    // Create output filename
    let output_file_name = match day {
        Some(d) => format!("tiny_desk_listings_{}_{}_{}.json", year, month, d),
        None => format!("tiny_desk_listings_{}_{}.json", year, month),
    };
...
fn main() -> Result<()> {
    ...
    if !month.chars().all(char::is_numeric) || month.len() != 2 || 
       month.parse::<u32>().unwrap_or(0) < 1 || month.parse::<u32>().unwrap_or(0) > 12 {
        eprintln!("Month must be in MM format (e.g., 01 for January)");
        std::process::exit(1);
    }
    
    if let Some(d) = day {
        if !d.chars().all(char::is_numeric) || d.len() != 2 || 
           d.parse::<u32>().unwrap_or(0) < 1 || d.parse::<u32>().unwrap_or(0) > 31 {
            eprintln!("Day must be in DD format (e.g., 01 for the 1st)");
            std::process::exit(1);
        }
